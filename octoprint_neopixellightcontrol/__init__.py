...
class NeopixellightcontrolPlugin(octoprint.plugin.SettingsPlugin,
    ...
    def init_rgb(self):
        try:
                ...
                self._logger.info("count %s" % self._settings.get_int(["led_count"]))
                self._logger.info("pin %s" % self._settings.get_int(["color_pin"]))
                self._logger.info("freq %s" % self._settings.get_int(["freqHz"]))
                self._logger.info("dma %s" % self._settings.get_int(["led_dma"]))
                self._logger.info("invert %s" % self._settings.get_boolean(["invert"]))
                self._logger.info(
